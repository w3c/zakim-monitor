...
# part_channel -- leave a channel, the channel name is give as argument
sub part_channel
{
  my ($self, $channel) = @_;
...
# nick_strip -- return the nickname part from a raw nick
sub nick_strip
{
  # Need to override the version in Bot::BasicBot, because it doesn't
  # take into account the case that the "!user@domain" part is missing
  # from the raw nick. Which may happen if the nick is the same user
  # as us and connected to the same IRC proxy as us.
  my $self = shift;
  my $combined = shift || "";
  return $combined =~ s/!.*//r;
}
